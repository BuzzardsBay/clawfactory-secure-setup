...
- **Pull requests for security fixes and documented bugs.** Match the existing code style (PowerShell 5.1 compatible, no PS7-only syntax). Every PR must compile cleanly with `ISCC.exe ClawFactory-Secure-Setup.iss` and pass the smoke test on a clean Windows 11 VM. Add a row to the smoke-test history in `CLAUDE_ClawFactory.md` for any change that affects install behavior.
