...
function Invoke-WslFile {
    ...
    $unc  = "\\wsl$\$script:CfDistro\tmp\cfprobe-$Tag.sh"
    $lin  = "/tmp/cfprobe-$Tag.sh"
    $out  = "C:\cfv\wsl\$Tag.out"
    New-Item -ItemType Directory -Path 'C:\cfv\wsl' -Force | Out-Null

    # UTF8 without BOM. A BOM at the head of a shell script is a syntax error on
    # line 1, and the resulting failure looks like the script's fault, not the
    # writer's (same family as the PS 5.1 BOM bug that broke the signtool dlib).
    ...
    # from a pipe. cmd /c owns the redirect; wsl.exe gets a bare path argument.
    cmd.exe /c "wsl.exe -d $script:CfDistro -u $User -- /bin/bash $lin > `"$out`" 2>&1"
    $rc = $LASTEXITCODE
