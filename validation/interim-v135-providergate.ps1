...
if ($RunFullInstallControl) {
    ...
    # THE RIG MUST SURVIVE THE INSTALL'S OWN DISTRO RESTART, AND UNTIL cfv-180 IT
    # DID NOT.
    #
    # An /etc/hosts entry alone is erased before the gate ever runs. Measured, not
    # guessed (interim-v144-hostsdiag.ps1): /etc/wsl.conf sets no generateHosts,
    # WSL's default is to regenerate /etc/hosts on every distro start, the shipped
    # file carries WSL's own "automatically generated by WSL" header naming that
    # exact setting, and setup.ps1 runs `wsl --shutdown` at Step 4 -- on cfv-180,
    # three seconds after the rig landed. The rig landed, PG.3.CTL fired, and nine
    # minutes later the gate resolved the real address and passed.
    #
    # This header already explains why a FIREWALL-rule rig was rejected:
    # Step-EgressFirewall rewrites the ruleset, so the re-run would undo it. The
    # same objection applies to /etc/hosts for a different reason, and it was
    # missed. Both candidate rigs are erased by the very install they exist to
    # interrupt.
    #
    # generateHosts=false is written FIRST so the restart cannot regenerate the
    # file, and it is inserted into the EXISTING [network] section -- a second
    # [network] section would be ambiguous, and setup.ps1's own wsl.conf edit
    # rewrites only [user], so this survives its Step 5b.
    #
