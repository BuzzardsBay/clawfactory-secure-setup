...
## 12. `PG.3f`: NOT CLOSED ON BOX B — and this is the most useful thing box B produced

**`PG.3f` has never been closed in this project.** Box B is the first run to
attempt it, and it did not close. The reason is an instrument limitation, it is now
root-caused from a measurement, and it is fixed and calibrated.

### 12.1 What happened

```
[18:37:52] RESTORED=160.79.104.10          <- level-1 rig removed
[18:37:53] L2_RIG=192.0.2.1                <- level-2 rig landed, PG.3.CTL fired
[18:37:56] Step 4: Restarting WSL.
[18:37:58] wsl --shutdown exit=0
[18:46:31] Step 15h: Provider-route gate - TCP connect to api.anthropic.com:443 as clawuser
[18:46:31] Provider-route gate PASSED: clawuser reached api.anthropic.com:443.
[18:46:42] INSTALLER_DONE=success
[18:46:42] LEVEL2_STATE=COMPLETED
```

**The fault landed and was then erased by the install itself, nine minutes before
the gate ran.**

### 12.2 CLAUSE 2 IS THE REASON THIS IS A DIAGNOSIS AND NOT A FALSE SHIP-BLOCKER

**Under the old row, this run reports `PG.3c FAIL`: "the installer did NOT refuse
rather than completing".** Read literally, on a release-validation box, that says
*the provider-route gate does not abort the install* — a security control failing
open. It would have been the fourth false finding of this cycle and the most
alarming one.

The classified form records:

```
LEVEL2_STATE=COMPLETED
LEVEL2_MARKER=[2026-08-28 18:46:42] [INFO] INSTALLER_DONE=success
PG.3c  state=COMPLETED  (GATE_ABORT is the pass; OTHER_ABORT means the rig stopped
       the install somewhere earlier; COMPLETED means the rig did not stop it at
       all; NO_MARKER means nothing was measured)
```

`COMPLETED` points at the rig. `FAIL` points at the product. **Same run, same
bytes, opposite conclusions**, and the difference is a named state set instead of a
string test. Diagnosis took four minutes.

### 12.3 The cause, measured rather than inferred

The obvious explanation — WSL regenerates `/etc/hosts` — was written down as an
*inference* and then replaced with a reading, because the fix depends on which
cause it is. `interim-v144-hostsdiag.ps1` on the box:

```
=== generateHosts explicitly set? (absent means the WSL default, which is true) ===
GENERATEHOSTS_NOT_SET
=== /etc/hosts, first lines ===
...
# [network]
# generateHosts = false
WSL_GENERATED_HEADER=yes
```

**Cause established.** One reading is explicitly *not* claimed as evidence: the
`hosts_mtime` is 502 s after boot, but `PG.3e` had already restored the file from
its backup by then, so that number is confounded by this probe's own cleanup and
proves nothing. Named rather than quietly included.

### 12.4 The finding worth carding, which is about the probe's own reasoning

`interim-v135-providergate.ps1`'s header explains at length why a **firewall-rule**
rig was rejected:

> *"A firewall edit was rejected because Step-EgressFirewall rewrites the ruleset,
> so a rigged rule would be undone by the very re-run that is supposed to observe
> it, and the gate would pass for the wrong reason. /etc/hosts survives that."*

**The reasoning is right about the firewall and wrong about `/etc/hosts`.** It
survives `Step-EgressFirewall`; it does not survive `Step 4: Restarting WSL`. The
author checked one erasure mechanism and stopped. **Both candidate rigs are erased
by the very install they exist to interrupt** — for different reasons, which is
exactly why checking one did not reveal the other.

### 12.5 The fix, and it was calibrated BEFORE another install was spent on it

The question *"can any rig survive?"* was answered in **one minute** by
`interim-v144-rigdurable.ps1` rather than by another ten-minute install — which is
what cfv-180 had just done, and it left the question unanswered either way.

```
RD.CTL0  PASS  the provider resolves to a real address BEFORE the rig
                RESOLVES_BEFORE=160.79.104.10
RD.CTL1  PASS  THE FAULT LANDED before the restart
                GENHOSTS_AFTER_WRITE=1  RESOLVES_AFTER_WRITE=192.0.2.1
RD.CTL2  PASS  the distro really restarted: boot_id changed
                244b8e05-9a7a-427f-a971-f8f3793c1864 -> 80337e8f-8aaa-45e1-bafe-583d7c4894f8
RD.1     PASS  a generateHosts=false rig SURVIVES the distro restart
                HOSTS_ENTRY_SURVIVED=1  RESOLVES_AFTER_RESTART=192.0.2.1
RD.2     PASS  the box is left byte-identical to how it was found
                WSLCONF_SHA before == after; rig lines 0; provider resolves 160.79.104.10

PASS=6 FAIL=0 VOID=0  controls fired=4/4
```

**`RD.CTL2` is the row that makes `RD.1` mean anything.** Without a proven restart,
an unchanged `/etc/hosts` proves only that nothing happened.

The level-2 rig now writes `generateHosts=false` into the **existing** `[network]`
section first, then the address entry. `PG.3.CTL` requires **all three** conditions
— unroutable resolution, `generateHosts` written, exactly one `[network]` section —
because on cfv-180 the address rig *alone* fired that control over a fault about to
vanish. `PG.3e` restores and reads back both files.

### 12.6 Why `PG.3f` moves to box C rather than being retried on box B

Two independent reasons, and both would have to be fixed to take it here:

1. **The rig.** Fixed above, but unproven against a real install.
2. **`PG.2.PRE` cannot be met on box B at all.** Box B installed with the provider
   deferred, so the log carries no gate verdict, the precondition is correctly
   unmet, and the phase VOIDs — **downgrading the level-2 rows with it**, exactly
   as it did on this run (`PG.3c` and `PG.3d` are annotated *"was FAIL, voided with
   the phase"*). Even a perfect level-2 result would be uncertifiable here.
