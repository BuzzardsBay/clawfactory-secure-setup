## SHIP STATUS: v1.0.16 — 2026-05-08 — STABLE (3 PASS + 1 harness-flake across 4 Azure validation cycles)
...
| `/v1/chat/completions` returned HTTP 404 on every install since v1.0.1 — chatCompletions route never registered | Added `--strict-json` to four `openclaw config set` calls for non-string types (gateway.port x2, plugins.entries.bonjour.enabled, gateway.http.endpoints.chatCompletions.enabled); without it openclaw silently drops boolean/integer writes while still returning exit 0 | v1.0.16 — TBD |
...
**v1.0.16 diagnosis lineage — chatCompletions route never registered.** Phase 3 RDP investigation confirmed openclaw 2026.4.27 requires `--strict-json` for non-string config values. Without it, the value is treated as a raw string, fails schema validation, and is silently discarded while exit 0 is still returned. The chatCompletions enabled flag was never written on any install from v1.0.1 onward — this is the first fix. Schema audit (`schema-investigation.log`, gitignored) confirms the path is valid; the flag alone was missing. Also fixed: `gateway.port` (integer) and `plugins.entries.bonjour.enabled` (boolean) for correctness, though those happened to produce correct observable behavior due to matching defaults.
...
| TBD | TBD (v1.0.16) | pending | chatCompletions probe added to Phase 3 verification — pending v1.0.16 Azure validation cycle |
